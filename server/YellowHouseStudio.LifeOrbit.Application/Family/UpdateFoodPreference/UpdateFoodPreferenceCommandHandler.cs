...
public class UpdateFoodPreferenceCommandHandler : IRequestHandler<UpdateFoodPreferenceCommand, FamilyMemberResponse>
{
    ...
    private readonly ILogger<UpdateFoodPreferenceCommandHandler> _logger;

    public UpdateFoodPreferenceCommandHandler(
        ApplicationDbContext context,
        ICurrentUser currentUser,
        ILogger<UpdateFoodPreferenceCommandHandler> logger)
    {
        _context = context;
    ...
    public async Task<FamilyMemberResponse> Handle(UpdateFoodPreferenceCommand request, CancellationToken cancellationToken)
    {
        _logger.LogDebug("Updating food preference {FoodItem} for family member {FamilyMemberId}", request.FoodItem, request.FamilyMemberId);
        ...
        familyMember.AddOrUpdatePreference(request.FoodItem, Enum.Parse<PreferenceStatus>(request.Status));
